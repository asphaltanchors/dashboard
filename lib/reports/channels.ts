...
// Product-specific sales channel metrics
export async function getProductSalesChannelMetrics(productCode: string, filters?: FilterParams): Promise<SalesChannelMetric[]> {
  const today = new Date()
  const days = filters?.dateRange ? parseInt(filters.dateRange.replace("d", "")) : 365
