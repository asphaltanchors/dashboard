...
// Add 'range' prop for link generation
interface Props {
  metrics: SalesChannelMetric[]
  range: string // Added range prop
  // onChannelClick?: (channel: string) => void // Removed onChannelClick
}
...
// Updated Props to include 'range'
export default function SalesChannelTable({ metrics, range }: Props) {
      ...
      <Table>
        ...
        <TableBody>
          {sortedMetrics.map((metric) => {
            ...
            const channelId = encodeURIComponent(metric.sales_channel || 'unknown');
            ...
            return (
              ...
              >
                {/* Wrap channel name in Link */}
                <TableCell className="font-semibold">
                  <Link
                    href={`/channels/${channelId}?range=${range}`}
          ...
          <TableRow className="border-t-2 bg-gray-50/50 font-bold">
            {/* Removed onClick and related cn logic from Total row */}
